...
import { MockCompound } from "../typechain-types";
...
describe("MockCompound", function () {
  ...
  describe("Deployment", function () {
    ...
    it("Should initialize with zero balance for any user", async function () {
      expect(await mockCompound.balanceOf(user1.address)).to.equal(0);
    });
  });
  ...
  describe("Deposit Function", function () {
    ...
    it("Should update total deposits", async function () {
      ...
      await mockCompound.connect(user1).deposit({ value: depositAmount });
    ...
    it("Should allow multiple deposits from same user", async function () {
      ...
      await mockCompound.connect(user1).deposit({ value: deposit1 });
      await mockCompound.connect(user1).deposit({ value: deposit2 });

      expect(await mockCompound.balanceOf(user1.address)).to.equal(deposit1 + deposit2);
      expect(await mockCompound.totalDeposits()).to.equal(deposit1 + deposit2);
    });
    ...
    it("Should handle deposits from multiple users", async function () {
      ...
      await mockCompound.connect(user1).deposit({ value: deposit1 });
      await mockCompound.connect(user2).deposit({ value: deposit2 });

      expect(await mockCompound.balanceOf(user1.address)).to.equal(deposit1);
      expect(await mockCompound.balanceOf(user2.address)).to.equal(deposit2);
      expect(await mockCompound.totalDeposits()).to.equal(deposit1 + deposit2);
    });
    ...
    it("Should correctly track contract balance", async function () {
      ...
      const contractBalance = await ethers.provider.getBalance(await mockCompound.getAddress());
  ...
  describe("Withdraw Function", function () {
    ...
    it("Should allow user to withdraw their deposit", async function () {
      ...
      expect(await mockCompound.balanceOf(user1.address)).to.equal(depositAmount - withdrawAmount);
    });

    it("Should update total deposits after withdrawal", async function () {
      const withdrawAmount = ethers.parseEther("5");

      await mockCompound.connect(user1).withdraw(withdrawAmount);

      expect(await mockCompound.totalDeposits()).to.equal(depositAmount - withdrawAmount);
    });
    ...
    it("Should revert if withdrawal amount exceeds balance", async function () {
      ...
      await expect(mockCompound.connect(user1).withdraw(withdrawAmount)).to.be.revertedWith("Insufficient balance");
    });

    it("Should revert if user has no deposits", async function () {
      await expect(mockCompound.connect(user2).withdraw(ethers.parseEther("1"))).to.be.revertedWith(
        "Insufficient balance",
      );
    });

    it("Should allow full withdrawal", async function () {
      await mockCompound.connect(user1).withdraw(depositAmount);
    ...
    it("Should allow multiple partial withdrawals", async function () {
      ...
      await mockCompound.connect(user1).withdraw(withdraw1);
      await mockCompound.connect(user1).withdraw(withdraw2);

      expect(await mockCompound.balanceOf(user1.address)).to.equal(depositAmount - withdraw1 - withdraw2);
    });

    it("Should not affect other users' balances", async function () {
      const user2Deposit = ethers.parseEther("5");
      await mockCompound.connect(user2).deposit({ value: user2Deposit });

      await mockCompound.connect(user1).withdraw(ethers.parseEther("3"));

      expect(await mockCompound.balanceOf(user2.address)).to.equal(user2Deposit);
    });
  });
  ...
  describe("Receive Function", function () {
    ...
    it("Should update total deposits on direct transfer", async function () {
      ...
      expect(await mockCompound.totalDeposits()).to.equal(amount);
    });
    ...
    it("Should handle multiple direct transfers", async function () {
      ...
      expect(await mockCompound.balanceOf(user1.address)).to.equal(amount1 + amount2);
    });
  });
  ...
  describe("Balance Tracking", function () {
    it("Should correctly track balances for multiple users", async function () {
      ...
      await mockCompound.connect(user1).deposit({ value: deposit1 });
      await mockCompound.connect(user2).deposit({ value: deposit2 });
      await mockCompound.connect(owner).deposit({ value: deposit3 });

      expect(await mockCompound.balanceOf(user1.address)).to.equal(deposit1);
      expect(await mockCompound.balanceOf(user2.address)).to.equal(deposit2);
      expect(await mockCompound.balanceOf(owner.address)).to.equal(deposit3);
      expect(await mockCompound.totalDeposits()).to.equal(deposit1 + deposit2 + deposit3);
    });
    ...
    it("Should maintain accurate balances after deposits and withdrawals", async function () {
      ...
      await mockCompound.connect(user2).deposit({ value: ethers.parseEther("5") });

      await mockCompound.connect(user1).withdraw(ethers.parseEther("3"));

      expect(await mockCompound.balanceOf(user1.address)).to.equal(ethers.parseEther("7"));
      expect(await mockCompound.balanceOf(user2.address)).to.equal(ethers.parseEther("5"));
      expect(await mockCompound.totalDeposits()).to.equal(ethers.parseEther("12"));
    });
  });
  ...
  describe("Edge Cases", function () {
    it("Should handle very small deposits", async function () {
      ...
      await mockCompound.connect(user1).deposit({ value: smallAmount });

      expect(await mockCompound.balanceOf(user1.address)).to.equal(smallAmount);
    });

    it("Should handle large deposits", async function () {
      const largeAmount = ethers.parseEther("100");

      await mockCompound.connect(user1).deposit({ value: largeAmount });

      expect(await mockCompound.balanceOf(user1.address)).to.equal(largeAmount);
    });

    it("Should handle zero balance withdrawal attempt", async function () {
      await expect(mockCompound.connect(user1).withdraw(1)).to.be.revertedWith("Insufficient balance");
    });
    ...
    it("Should handle deposit and immediate full withdrawal", async function () {
      ...
      await mockCompound.connect(user1).withdraw(amount);
